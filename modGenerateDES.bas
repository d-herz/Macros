...
' Estimate Sheets (DES) used in the Standard Cost Estimate workbook. It manages
' the creation, population, and formatting of DES worksheets based on selected
' inputs, estimate data, and predefined template rules.
'
' Responsibilities may include:
'   - Creating new DES sheets
'   - Naming and organizing DES worksheets consistently
'   - Populating quantities, units, and calculated values
'   - Applying protections, formatting, and structural rules
'
' Notes:
' Procedures in this module are workflow-driven and may rely on shared utility
' functions and workbook state. Changes to this module can have wide-reaching
' impacts on estimate structure and downstream calculations and should be
' tested carefully.
'===============================================================================
...
Sub GenerateDES_Letter()
    Call GenerateDES(desLetter, True)
    DESOutOfDate = False
End Sub

Sub GenerateDES_11x17()
    Call GenerateDES(des11x17, True)
    DESOutOfDate = False
End Sub


Sub GenerateDES(layout As DESLayoutType, Optional ByVal ExportPrompt As Boolean = True)
    ' -------------------------
    ' SPEED SWITCHES ON
    ' -------------------------
    FreezeUI
    ...
    ' Determine max columns and footer inclusion based on layout
    Select Case layout
        Case desLetter
            maxColumns = 26
            includeFooter = True
    ...
    Call Apply_Print_Settings(wsDES, layout)
    ...
    ' --- Finalize the very last category header after loop ends ---
...
Sub Apply_Print_Settings(ws As Worksheet, layout As DESLayoutType)
    With ws.PageSetup
...
' Helper for loggging/tracking the size DES layout generated
